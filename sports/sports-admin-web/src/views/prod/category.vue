...
import { getProdCategoryList, addProdCategory, updateProdCategory, deleteProdCategory } from '../../api/prod'
...
const handleSubmit = async () => {
  ...
  if (!valid) return
  if (editingId.value) {
    await updateProdCategory({ ...form, id: editingId.value })
  } else {
    const submitData = { ...form }
    delete submitData.id  // 新增时删除id，让数据库自动生成
    await addProdCategory(submitData)
  }
...
const handleDelete = async (row) => {
  ...
  await deleteProdCategory(row.id)
